...
关于面向对象、泛型、鸭子类型等概念详见[面向对象](/golang/other/oo/)
...
!!! note
	上面`t := Teacher{}`可以用`t := new(Teacher)`替代，但这2者是有一点区别的，区别在于前者是指针，详见[new](/golang/other/new/#new)
...
更多关于struct嵌套的知识，详见[struct作为struct的元素](/golang/datatype/struct_struct_element/)
...
详细剖析上面的tar.gz例子，详见[golang-targz脑图](/golang/attachment/golang_targz.itmz)（点击下载，并使用iThoughtsX打开）
